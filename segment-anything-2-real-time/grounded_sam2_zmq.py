        ...
        while True:
            ...
            for oid, mask in current_masks.items():
                ...
                if centroid and depth_m is not None:
                    ...
                    # Collect valid depth values in a region around the centroid
                    # Uses median for robustness (handles translucent/reflective objects)
                    search_radius = 25
                    valid_points = []
                    for dy in range(-search_radius, search_radius + 1, 5):
                        for dx in range(-search_radius, search_radius + 1, 5):
                            ...
                            d = depth_m[ny, nx]
                            if 0.05 < d < 2.0:
                                p = cam.pixel_to_3d(nx, ny, depth_m)
                                if p is not None:
